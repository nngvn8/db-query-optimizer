...
Generates Protobuf plan binary files for all queries in the specified directory
using optimizer-db-client in interactive standalone mode.
...
Generated files:
  Protobuf plan files (.pb) are written to generated/pb_plans/
...
CLIENT_ARGS=("-standalone" "-writeProto" "-matType" "${MAT_TYPE}")
...
echo ""
echo "Protobuf plan generation completed successfully."
