...
namespace {
    ...
    // --- IR Label Generation ---
    std::string getIrLabel(const PlanNode& node, bool shortColumns) {
    ...
    std::string getApiLabel(const PlanNode& node, bool shortColumns) {
        HtmlLabelBuilder builder;
    ...
    void writePlanNodeDot(const PlanNode* node, std::ofstream& file, DotContentType contentType, std::unordered_set<const PlanNode*>& visited, bool shortColumns, bool ignoreFetch) {
        if (!node) return;
        ...
        if (contentType == DotContentType::IR_DATA) {
            label = getIrLabel(*node, shortColumns);
        } else {
            label = getApiLabel(*node, shortColumns);
        }
        ...
        for (const auto& child : node->children) {
            if (child) {
                ...
                // Recurse
                writePlanNodeDot(child.get(), file, contentType, visited, shortColumns, ignoreFetch);
            }
        }
    }
} // namespace

void generatePlanDotFile(const PlanNode& root, const std::string& filename, DotContentType contentType, bool shortColumns, bool ignoreFetch) {
    ...
    writePlanNodeDot(&root, file, contentType, visited, shortColumns, ignoreFetch);
